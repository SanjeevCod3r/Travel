...
export const FleetShowcase = ({
  ...
  const getVehicleIcon = (type) => {
    ...
    if (typeLower.includes("sedan") || typeLower.includes("luxury"))
      return Crown;
    if (
      typeLower.includes("suv") ||
      typeLower.includes("tempo") ||
      typeLower.includes("muv")
    )
      return Users;
    return Car;
  };

  const displayVehicles = vehicles.slice(0, 4).map((v) => ({
    ...v,
    category: v.type
      ? v.type.charAt(0).toUpperCase() + v.type.slice(1)
      : "Premium Vehicle",
    seats: `${v.seating || 4} Passengers`,
    features: (v.features || [])
      .slice(0, 3)
      .concat(
        (v.features || []).length < 3
          ? ["Premium Interior", "Comfortable Ride"].slice(
              0,
              3 - (v.features || []).length
            )
          : []
      ),
    image: v.images?.[0] || "/asset/car-placeholder.png",
  ...
  if (loading) {
    return (
      <section className="py-24 bg-gradient-to-br from-slate-50 via-white to-paleBlue-50 relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 md:px-12 grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {[...Array(4)].map((_, i) => (
            <div
              key={i}
              className="animate-pulse bg-white/50 backdrop-blur-sm rounded-3xl h-[400px] border border-gray-100/50"
            />
          ))}
        </div>
      </section>
  ...
  return (
    <section className="py-24 bg-gradient-to-br from-slate-50 via-white to-paleBlue-50 relative overflow-hidden">
      ...
      <div className="absolute inset-0">
        <div className="absolute top-10 left-10 w-20 h-20 bg-gradient-to-r from-paleBlue-400/10 to-purple-400/10 rounded-full blur-xl animate-pulse"></div>
        ...
        <div className="absolute top-20 right-1/4 w-16 h-16 bg-gradient-to-r from-cyan-400/10 to-paleBlue-400/10 rounded-full blur-xl animate-pulse delay-700"></div>
      </div>
          ...
          <div className="relative z-10 px-8 py-12 md:px-12 md:py-16 text-center text-white">
            ...
            >
              <motion.button
                ...
                style={{ fontFamily: "Montserrat, sans-serif" }}
              >
                <div className="absolute inset-0 bg-gradient-to-r from-[#0056D2] to-[#4A8BDF] opacity-0 hover:opacity-10 transition-opacity duration-300" />
                <Car size={18} />
                View All Vehicles
